<template>
    <div>
        <button @click="vote('up')">
            <svg 
                :class="{'fill-current text-green-600' : upvoted}"
        ...
        <button @click="vote('down')">
            <svg
               :class="{'fill-current text-green-600' : downvoted}"
                version="1.1"
                id="Capa_1"
                xmlns="http://www.w3.org/2000/svg"
...
export default {
    ...
    data() {
        return { votes: this.default_votes };
    },
    computed: {
        upvotes() {
    ...
    methods: {
        vote(type){
            ...
            {
                axios.delete(`/votes/${this.entity_id}/${type}`)
                .then(()=>{
